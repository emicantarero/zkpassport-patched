...
export function fromBytesToBigInt(bytes) {
    return BigInt("0x" + Buffer.from(bytes).toString("hex"));
}
export function fromArrayBufferToBigInt(buffer) {
    return BigInt("0x" + Buffer.from(buffer).toString("hex"));
}
export function rightPadArrayWithZeros(array, length) {
    return array.concat(Array(length - array.length).fill(0));
}
export function leftPadArrayWithZeros(array, length) {
    return Array(length - array.length)
        .fill(0)
        .concat(array);
}
export function getBitSize(number) {
    return number.toString(2).length;
}
export function getOffsetInArray(array, arrayToFind, startPosition = 0) {
...
export function assert(truthy, errorMsg) {
    if (!truthy) {
        throw new Error(errorMsg);
    }
}
export function packBeBytesIntoField(x, maxFieldSize) {
    let result = BigInt(0);
 ...
 */
export function packBeBytesIntoFields(bytes, maxChunkSize) {
    if (bytes.length === 0)
        return [];
...
export { PromisePool, AggregateError } from "./promise-pool.js";
